        ...
        try:
            ...
            for word in settingsmongo.TRACK_TERMS:        # loop to pick out the filtered word in the TWEET (text) ONLY
                if td.get('text') and word in td['text']:
                    ...
                    if word in settingsmongo.Conservative:
                        td['Bin'] = 'Conservative'
                    elif word in settingsmongo.LibDem:
                        td['Bin'] = 'LibDem'
                    elif word in settingsmongo.Labour:        # to create 'bin' term for each filter word
                        td['Bin'] = 'Labour'
                    ...
                    db.test_tweets.insert(td)  # insert all columns into mongo
                    print '___: D___'
